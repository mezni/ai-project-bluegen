...
class PromptManager:
    ...
    def _load_config(self) -> dict:
        if not self.config_path.exists():
            raise FileNotFoundError(
                f"Prompt configuration not found: {self.config_path}"
            )
    ...
    def get_system_prompt(self) -> str:
        project_config = self.config["project_blueprint"]

        version = project_config["version"]
        base_path = Path(project_config["path"])

        prompt_path = base_path / version / "system.txt"
    ...
    def build_user_prompt(self, project_idea: str) -> str:
        ...
        prompt_path = base_path / version / "user.txt"
    ...
    def get_version(self) -> str:
        return self.config["project_blueprint"]["version"]
